...
const login = async (
    username: string,
    password: string,
): Promise<ApiResponse<AuthLoginResponseType> | null> => {
    const response = await $api.post<ApiResponse<AuthLoginResponseType>>(
        `${BASE_AUTH_URL}/login`,
        undefined,
        {
...
const logout = async (): Promise<ApiResponse<void> | null> => {
